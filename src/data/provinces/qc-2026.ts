/**
 * Quebec income tax parameters, tax year 2026.
 *
 * Quebec is the reason this project refused to approximate it. Four things
 * differ structurally from every other jurisdiction:
 *
 * 1. **QPP instead of CPP**, and **QPIP**, which has no equivalent elsewhere.
 *    Those live in `quebec-payroll-2026.ts`.
 * 2. **The federal abatement.** A Quebec resident's federal tax is reduced by
 *    16.5%, because Quebec runs programs the federal government runs
 *    elsewhere. Omitting it would overstate their total tax substantially.
 * 3. **The deduction for workers.** 6% of employment income up to 1,450,
 *    deducted from Quebec taxable income. It is Quebec's replacement for the
 *    contribution credits the rest of Canada grants, which is why Revenu
 *    Québec's own formula has no QPP, EI or QPIP credit term.
 * 4. **Its own tax administration.** These rates come from Revenu Québec and
 *    the Ministère des Finances, not from CRA.
 */

import type { ProvincialParameters } from './types.ts';

/** Revenu Québec, "Income tax rates". */
const RQ_RATES =
  'https://www.revenuquebec.ca/en/citizens/income-tax-return/completing-your-income-tax-return/income-tax-rates/';

/** Revenu Québec, TP-1015.F-V, "Formulas to Calculate Source Deductions", 2026-01. */
const RQ_FORMULAS =
  'https://www.revenuquebec.ca/documents/en/formulaires/tp/TP-1015.F-V(2026-01).pdf';

/** Ministère des Finances du Québec, "Parameters of the Personal Income Tax System for 2026". */
const MFQ_PARAMETERS =
  'https://cdn-contenu.quebec.ca/cdn-contenu/adm/min/finances/publications-adm/parametres/AUTEN_IncomeTax2026.pdf';

/** CRA, "Payroll Deductions Formulas", 122nd edition, effective 2026-01-01. */
const CRA_T4127 =
  'https://www.canada.ca/en/revenue-agency/services/forms-publications/payroll/t4127-payroll-deductions-formulas/t4127-jan/t4127-jan-payroll-deductions-formulas-computer-programs.html';

const RETRIEVED = '2026-08-25';

export const QUEBEC_2026: ProvincialParameters = {
  code: 'QC',
  name: 'Quebec',

  brackets: {
    value: [
      { from: 0, to: 54_345, rate: 0.14 },
      { from: 54_345, to: 108_680, rate: 0.19 },
      { from: 108_680, to: 132_245, rate: 0.24 },
      { from: 132_245, to: null, rate: 0.2575 },
    ],
    source: RQ_RATES,
    retrieved: RETRIEVED,
    note: 'Thresholds corroborated by the Ministère des Finances parameter tables and by TP-1015.F.',
  },

  creditRate: {
    value: 0.14,
    source: RQ_FORMULAS,
    retrieved: RETRIEVED,
    note: 'Personal credits convert at 14%, which is also the lowest bracket rate.',
  },

  basicPersonalAmount: {
    value: 18_952,
    source: MFQ_PARAMETERS,
    retrieved: RETRIEVED,
    note: 'Table 3. Indexed by 2.05% from 18,571 in 2025. The largest personal amount in Canada.',
  },

  // Revenu Québec's own annual formula has no QPP, EI or QPIP credit term:
  // the deduction for workers replaced them. Granting both would double count.
  grantsContributionCredits: false,

  workerDeduction: {
    rate: {
      value: 0.06,
      source: RQ_FORMULAS,
      retrieved: RETRIEVED,
      note: 'Factor H: 6% of employment income, deducted from Quebec taxable income.',
    },
    maximum: {
      value: 1_450,
      source: MFQ_PARAMETERS,
      retrieved: RETRIEVED,
      note: 'Table 3, maximum amount of the deduction for workers.',
    },
  },

  federalAbatementRate: {
    value: 0.165,
    source: CRA_T4127,
    retrieved: RETRIEVED,
    note: 'Federal tax for a Quebec resident is reduced by this, per formula T1.',
  },
};
